...
def get_config_dir(config_dir: str | Path | None = None) -> Path:
    ...
    if env_dir := os.getenv("GEOMCP_CONFIG_DIR"):
...
def load_config(config_dir: str | Path | None = None) -> dict[str, Any]:
    ...
    for name in OPTIONAL_CONFIG_FILES:
        path = directory / name
        config[Path(name).stem] = _load_yaml(path) if path.is_file() else {}
    config["_meta"] = {
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if overlap:
        raise ConfigurationError(
            f"Capabilities cannot be both allowed and denied: {', '.join(sorted(overlap))}"
        )
    ...
    if not isinstance(workspaces, dict):
        raise ConfigurationError("workspaces.workspaces must be a mapping")
    ...
    for name, spec in workspaces.items():
        ...
        if not any(_within(resolved_read, root) for root in read_bounds):
            raise ConfigurationError(
                f"Workspace {name} read_root is outside paths.read_roots: {resolved_read}"
            )
        if not any(_within(resolved_write, root) for root in write_bounds):
            raise ConfigurationError(
                f"Workspace {name} write_root is outside paths.write_roots: {resolved_write}"
            )

    executors = config.get("executors", {})
    if not isinstance(executors, dict):
        raise ConfigurationError("executors configuration must be a mapping")
    gpu = executors.get("gpu", {})
    if gpu and not isinstance(gpu, dict):
        raise ConfigurationError("executors.gpu must be a mapping")
    if gpu.get("enabled"):
        required = ("host", "port", "username", "python", "config_dir")
        missing = [name for name in required if not gpu.get(name)]
        if missing:
            raise ConfigurationError(
                f"Enabled GPU executor is missing fixed endpoint fields: {', '.join(missing)}"
            )
