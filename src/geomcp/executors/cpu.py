...
class LocalCPUExecutor:
    def __init__(self, *, config_dir: str | Path | None=None):
        ...
        self.cpu = self.config.get("executors", {}).get("cpu", {}) or {}

    def submit(self, job_id: str) -> None:
        job = self.store.get(job_id)
        if job.executor != "cpu":
            raise ExecutorError("LocalCPUExecutor only accepts cpu jobs")
        ...
        try:
            ...
            self.store.transition(job_id, "failed", error=f"Unable to start CPU worker: {exc}")
        ...
        watch_dispatch(
            ...
            job_id,
            proc,
            timeout=float(self.cpu.get("dispatch_timeout", 10)),
            label="CPU launcher",
        )
